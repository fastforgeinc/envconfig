...
func TestEmptyMapFieldOverride(t *testing.T) {
	...
	os.Clearenv()
	_ = os.Setenv("ENV_CONFIG_REQUIREDVAR", "foo")
	_ = os.Setenv("ENV_CONFIG_MAPFIELD", "")
	if err := Process("env_config", &s); err != nil {
		t.Error(err.Error())
	}
...
func TestRequiredIfSet(t *testing.T) {
	type t1 struct {
		Property1 string `required:"true"`
		Property2 string
	}

	type t2 struct {
		RequiredProperty string `required:"true"`
		Property         string

		NestedRequired *t1 `required:"true"`
		Nested         *t1
	}

	os.Clearenv()
	_ = os.Setenv("ENV_CONFIG_REQUIREDPROPERTY", "value")
	err := Process("env_config", &t2{})
	if err == nil {
		t.Error("no failure when missing required variable")
	}
	if !strings.Contains(err.Error(), " ENV_CONFIG_NESTEDREQUIRED_PROPERTY1 ") {
		t.Errorf("expected error message to contain ENV_CONFIG_NESTEDREQUIRED_PROPERTY1, got \"%v\"", err)
	}

	_ = os.Setenv("ENV_CONFIG_NESTEDREQUIRED_PROPERTY1", "value")
	err = Process("env_config", &t2{})
	if err != nil {
		t.Errorf(err.Error())
	}

	_ = os.Setenv("ENV_CONFIG_NESTED_PROPERTY2", "value")
	err = Process("env_config", &t2{})
	if err == nil {
		t.Error("no failure when missing required variable")
	}
	if !strings.Contains(err.Error(), " ENV_CONFIG_NESTED_PROPERTY1 ") {
		t.Errorf("expected error message to contain ENV_CONFIG_NESTED_PROPERTY1, got \"%v\"", err)
	}

	_ = os.Setenv("ENV_CONFIG_NESTED_PROPERTY1", "value")
	err = Process("env_config", &t2{})
	if err != nil {
		t.Errorf(err.Error())
	}
